...
const rustCodeMask = (source) => {
  ...
  for (let offset = 0; offset < source.length; ) {
    if (source.startsWith("//", offset)) {
      ...
      const settledEnd = end === -1 ? source.length : end;
      blank(offset, settledEnd);
      offset = settledEnd;
      continue;
    }
    if (source.startsWith("/*", offset)) {
    ...
    const raw = /^(?:br|cr|r)(#*)"/.exec(source.slice(offset));
    if (
      raw &&
      (offset === 0 || !/[a-zA-Z0-9_]/.test(source[offset - 1]))
    ) {
      const terminator = `"${raw[1]}`;
      const end = source.indexOf(terminator, offset + raw[0].length);
      assert.notEqual(end, -1, "unterminated Rust raw string");
      const settledEnd = end + terminator.length;
      blank(offset, settledEnd);
      offset = settledEnd;
      continue;
    }
    ...
    const stringPrefix =
      source[offset] === '"'
        ? 0
        : (source[offset] === "b" || source[offset] === "c") &&
            source[offset + 1] === '"'
          ? 1
          : -1;
    if (stringPrefix !== -1) {
      let end = offset + stringPrefix + 1;
      ...
      assert.ok(terminated, "unterminated Rust string");
      blank(offset, end);
      offset = end;
      continue;
    }
    ...
    const characterStart =
      source[offset] === "'"
        ? offset
        : source[offset] === "b" && source[offset + 1] === "'"
          ? offset + 1
          : -1;
    if (characterStart !== -1) {
      let end = characterStart + 1;
      if (source[end] === "\\") {
        end += 1;
        if (source[end] === "x") {
          end += 3;
        } else if (source[end] === "u" && source[end + 1] === "{") {
          const brace = source.indexOf("}", end + 2);
          end = brace === -1 ? end : brace + 1;
        } else {
          end += 1;
        }
      } else {
...
const rustProductionSource = (source) => {
  const syntax = rustCodeMask(source);
  const testItem =
    /#\[\s*cfg\s*\(\s*test\s*\)\s*\](?:\s*#\[[^\]]*\]\s*)*\s*(?:(?:pub(?:\([^)]*\))?\s+)?(?:async\s+|const\s+|unsafe\s+|extern\s+"[^"]+"\s+)*fn\s+[a-zA-Z_][a-zA-Z0-9_]*|(?:pub(?:\([^)]*\))?\s+)?mod\s+[a-zA-Z_][a-zA-Z0-9_]*|impl\b)/g;
  const ranges = [];

  for (
    let match = testItem.exec(syntax);
    match;
    match = testItem.exec(syntax)
  ) {
    const remainder = syntax.slice(testItem.lastIndex);
    const delimiterOffset = remainder.search(/[;{]/);
    assert.notEqual(
      delimiterOffset,
      -1,
      "cfg(test) Rust item must have a body or semicolon",
    );
    const delimiter = testItem.lastIndex + delimiterOffset;
    if (syntax[delimiter] === ";") {
      ranges.push([match.index, delimiter + 1]);
      testItem.lastIndex = delimiter + 1;
      continue;
    }

    let depth = 0;
    let end = -1;
    for (let offset = delimiter; offset < syntax.length; offset += 1) {
      if (syntax[offset] === "{") depth += 1;
      if (syntax[offset] === "}") depth -= 1;
      if (depth === 0) {
        end = offset + 1;
        break;
    ...
    assert.notEqual(end, -1, "unterminated cfg(test) Rust item");
    ranges.push([match.index, end]);
    testItem.lastIndex = end;
  }

  if (ranges.length === 0) return syntax;
  const production = [...syntax];
...
test("Rust production masking cannot hide adjacent production mutations", () => {
  const production = rustProductionSource(`
fn production_write() {
    std::fs::write("production", b"visible");
}

#[cfg(test)]
fn test_write_with_lifetime<'a>(value: &'a str) -> char {
    let marker = 'x';
    std::fs::write("test", value);
    marker
}

#[cfg(test)]
mod tests {
    fn nested_test_remove() {
...
#[cfg(test)]
impl Fixture {
    fn test_impl_mutation(&self) {
        std::fs::create_dir_all("test");
    }
}

const FAKE_ITEM: &str = r#"#[cfg(test)] fn fake() {
    std::fs::rename("production", "hidden");
}"#;

fn production_remove() {
    std::fs::remove_file("production");
}
`);

  assert.match(production, /std::fs::write\s*\(/);
  assert.match(production, /std::fs::remove_file\s*\(/);
  assert.doesNotMatch(production, /std::fs::rename\s*\(/);
  assert.doesNotMatch(production, /std::fs::create_dir_all\s*\(/);
  assert.equal(
    [...production.matchAll(/std::fs::write\s*\(/g)].length,
    1,
    "only the production write remains visible",
  );
  assert.equal(
    [...production.matchAll(/std::fs::remove_file\s*\(/g)].length,
    1,
    "only the production remove remains visible",
  );
});
...
test("P01-B02 root acquisition retains exact partial-effect obligations", async () => {
  ...
  const windowsSetDeleteIndex = windowsCleanupRoot.indexOf(windowsSetDelete);
  const windowsPostDeleteProofIndex = windowsCleanupRoot.indexOf(
    windowsPostDeleteProof,
    windowsSetDeleteIndex + windowsSetDelete.length,
  );
  assert.ok(
    windowsPostDeleteProofIndex > windowsSetDeleteIndex,
    "Windows delete disposition before retained-object link-zero proof is out of order",
  );
...
test("P01-B02 native operations stay relative to retained handles", async () => {
  ...
  assert.match(
    windowsEntries,
    /(?:enumeration|listing|cursor)[a-z_]*\s*\.\s*lock\(\)/,
  );
...
test("P01-B02 tracks stage ownership through drop promotion and reset", async () => {
  ...
  const replaceStateName = replaceObligation.match(
    /(?:state|retained):\s*Option<(?:Box<)?([A-Za-z0-9_]+)(?:>)?>/,
  )?.[1];
        ...
        for (const branch of conditionalBlocks(block.source)) {
          ...
          for (const identifier of identifiers) {
            ...
            const update = new RegExp(
              `(?:\\blet\\s+(?:mut\\s+)?${escaped}\\s*=\\s*[^;]{0,240}\\b[a-z_]*${kind}[a-z0-9_]*\\b\\s*\\.\\s*checked_(?:add|sub)\\s*\\(|\\b${escaped}\\s*\\.\\s*checked_(?:add|sub)\\s*\\(|\\b${escaped}\\s*(?:\\+=|-=)\\s*1)`,
              "i",
            ).test(block.source);
            const boundedError =
...
test("P01-B02 resumes deterministic persisted-state parks as typed receipts off runtime", async () => {
  ...
  const restartEffect = new RegExp(
    `let\\s*\\(\\s*([a-z_][a-z0-9_]*)\\s*,\\s*([a-z_][a-z0-9_]*)\\s*\\)\\s*:\\s*\\(\\s*PersistedStateRepairTerminalOutcome\\s*,\\s*Option\\s*<\\s*AnchoredRecordQuarantinePreservationError\\s*>\\s*,?\\s*\\)\\s*=\\s*tokio::task::spawn_blocking`,
  ).exec(startup);
  assert.ok(
    restartEffect,
  ...
  const restartAckFailureMatch =
    /Err\s*\(\s*([a-z_][a-z0-9_]*)\s*\)\s*=>\s*(\(\s*PersistedStateRepairTerminalOutcome::AppliedUnverified\s*,\s*Some\s*\(\s*([a-z_][a-z0-9_]*)\s*\)\s*,?\s*\))/i.exec(
      restartAcknowledgement,
    );
  const restartAckFailure = restartAckFailureMatch
    ? {
        marker: `Err(${restartAckFailureMatch[1]})`,
        body: restartAckFailureMatch[2],
      }
    : undefined;
  assert.ok(
    restartAckFailure,
...
terminalTest("Application strictly decodes bounded static skin PNGs", async () => {
  const [nativeSkin, skinModule, skinImage, skinState, skinTests] =
    await Promise.all([
      read("apps/desktop/src/native_skin.rs"),
  ...
  assert.match(
    skinImage,
    /pub use crate::state::skins::SAVED_SKIN_PNG_MAX_BYTES as SKIN_PNG_MAX_BYTES/,
  );
  assert.match(
    skinState,
