...
test("P01-B01 keeps one explicit application-root authority", async () => {
  ...
  assert.match(
    rootSession,
    /pub struct AppRootResetAuthority \{[\s\S]*?authority: AppRootResetAttempt,[\s\S]*?reset_retry: Arc<Mutex<Option<AppRootResetRetry>>>/,
  );
  assert.match(
    rootSession,
  ...
  assert.match(
    rootSession,
    /impl AppRootResetAuthority \{[\s\S]*?pub fn clear_owned_root\(self\) -> io::Result<AppRootClearReceipt>/,
  );
  assert.match(
    desktopCommands,
    /root_session[\s\S]*?\.begin_reset\(\)[\s\S]*?clear_owned_root_off_runtime\(reset_authority\)[\s\S]*?cleared_root\.release\(\)/,
  );
  assert.match(
    desktopCommands,
    /async fn clear_owned_root_off_runtime\(\s*authority: axial_config::AppRootResetAuthority,[\s\S]*?spawn_blocking\(move \|\| authority\.clear_owned_root\(\)\)/,
  );
  assert.doesNotMatch(paths, /terminal_reset_scope|contains_resolved/);
  assert.doesNotMatch(
    desktopCommands,
    /terminal_reset_scope|contains_resolved/,
  );
  assert.doesNotMatch(
    desktopCommands,
  ...
  assert.match(
    state,
    /ManagedRuntimeCache::from_directory\(\s*root_session\.prepare_runtime_directory\(\)\?,\s*config\.paths\(\)\.runtimes_dir\(\)\.to_path_buf\(\)/,
  );
  assert.match(state, /config\.paths\(\) != init\.instances\.paths\(\)/);
  assert.match(state, /persisted_state_directories\.performance_parent\(\)/);
  assert.match(
    state,
  ...
  const exactConsumers = [
    ...
    [skins, /root_session\.prepare_saved_skin_directories\(\)/],
  ...
  assert.match(performanceRulesState, /claim_rules_authority\(\)/);
  assert.doesNotMatch(
    performanceRulesState,
