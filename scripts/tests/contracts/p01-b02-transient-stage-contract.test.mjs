...
function platformModule(source, target) {
  const marker = `#[cfg(${target})]\nmod native {`;
  const start = source.indexOf(marker);
  assert.notEqual(start, -1, `missing ${target} platform module`);
  if (target === "unix") {
    const end = source.indexOf("#[cfg(windows)]\nmod native {", start);
    assert.notEqual(end, -1, "missing Windows platform module");
...
test("transient admission batches reservation and one fresh inventory", async () => {
  ...
  assert.match(reserve, /operations\s*\.lock\(\)/);
  ...
  assert.equal([...settleBatch.matchAll(/operations\s*\.lock\(\)/g)].length, 1);
...
test("native transient publication uses the intended platform primitives", async () => {
  ...
  const windows = platformModule(platform, "windows");
...
test("monotonic transient batches classify and transfer exact authority", async () => {
  ...
  assert.match(
    settleTokens,
    /filter\(\|member\| member\.is_published\(\)\)\s*\.count\(\)/,
  );
  ...
  assert.doesNotMatch(
    platformModule(platform, "windows"),
