...
impl TransientEffectToken {
    fn reserve_batch(
        ...
        for (offset, record) in records.into_iter().enumerate() {
            let offset =
                u64::try_from(offset).expect("bounded transient destination offset fits in u64");
            let id = first_id
                .checked_add(offset)
                .expect("prechecked transient effect id range");
            ...
            debug_assert!(
                previous.is_none(),
                "prechecked transient effect id is vacant"
            );
            tokens.push(Self {
                id,
    ...
    fn settle_no_effect_batch(
        ...
        let mut state =
            first.authority.operations.lock().map_err(|_| {
        ...
        for token in tokens.iter() {
            ...
            debug_assert!(
                removed.is_some(),
                "prechecked transient effect is registered"
            );
        }
    ...
    fn settle_classified_batch(
        ...
        let published = members
            .iter()
            .filter(|member| member.is_published())
            .count();
        let outstanding_effects = state
            .outstanding_effects
    ...
    fn mark_disposition(&self, disposition: TransientEffectDisposition) -> io::Result<()> {
        ...
        if !matches!(
            record.phase,
            TransientEffectPhase::Reserved | TransientEffectPhase::Live
        ) {
            return Err(stale_capability());
        }
...
impl CapabilityAuthority {
    fn settle_transient_effect(&self, id: u64, operation: &CapabilityOperation) -> io::Result<()> {
    ...
    pub(super) fn cleanup_abandoned_transient(self: &Arc<Self>, id: u64) -> io::Result<()> {
        ...
        let result = match (
            record.disposition,
            record.identity,
            record.retained.as_ref(),
        ) {
            (TransientEffectDisposition::NoEffect, _, None) => Ok(()),
            (
                TransientEffectDisposition::Published | TransientEffectDisposition::Indeterminate,
                Some(identity),
                Some(retained),
        ...
        match platform::create_transient_file(&self.directory.inner.handle) {
            Ok((file, identity)) => {
                ...
                if let Err(error) = stage
                    ...
                    .mark_live(identity)
                {
                    return TransientStageCreateOutcome::Pending(TransientCreationObligation {
                        error,
...
impl Directory {
    pub fn admit_transient_destinations(
        ...
        let mut tokens = TransientEffectToken::reserve_batch(&authority, &operation, self, &plan)?;
        if let Err(error) = validate_destination_batch_with_operation(
            ...
            let cleanup = TransientEffectToken::settle_no_effect_batch(&mut tokens, &operation);
    ...
    pub fn admit_transient_destination(&self, name: LeafName) -> io::Result<TransientDestination> {
...
fn validate_destination_batch_with_operation(
    ...
    let mut inventory = DestinationBatchInventory {
        expected_exact: &expected_exact,
        exact: &mut exact,
        portable_key: &mut portable_key,
        native_key: &mut native_key,
        normalization: &mut normalization,
        collision_policy,
        directory_buffer: None,
    };
    validate_mixed_destination_batch_with_operation(directory, plan, &mut inventory, operation)?;
    Ok(exact)
}

struct DestinationBatchInventory<'a> {
    expected_exact: &'a [bool],
    exact: &'a mut [bool],
    portable_key: &'a mut Vec<u8>,
    native_key: &'a mut Vec<u8>,
    normalization: &'a mut Vec<(u8, char)>,
    collision_policy: DestinationCollisionPolicy,
    directory_buffer: Option<&'a mut [MaybeUninit<u8>]>,
}
...
fn validate_mixed_destination_batch_with_operation(
    ...
    let mut visit_entry = |observed_name: &std::ffi::OsStr, kind| {
        ...
        if let (Some(portable_target), Some(native_target)) = (portable_target, native_target)
        ...
        let error = if observed_name != target_name || exact[target] || kind != EntryKind::File {
...
impl Drop for TransientPublicationTransition {
    fn drop(&mut self) {
        ...
        if !armed {
            let _retained = self.retained.take();
            if let Some(stage) = self.stage.as_mut() {
                let _stage_file = stage.stage.file.take();
            }
            return;
        }
        if let Some(stage) = self.stage.as_mut() {
    ...
    pub fn publish_create_new(mut self) -> TransientPublicationBatchOutcome {
        ...
        for index in 0..self.stages.len() {
            ...
            match (link, state) {
                ...
                (Ok(()), _) => {
                    return pending_publication(io::ErrorKind::InvalidData.into(), self);
                }
            }
        }
        classify_publication_batch(io::ErrorKind::Other.into(), self, &operation)
    }
}
...
fn classify_publication_batch(
    ...
    for stage in &batch.stages {
        ...
        match transient_publication_state_for_publication(
            ...
            Ok(platform::TransientPublicationState::Indeterminate) => {
                return pending_publication(io::ErrorKind::WouldBlock.into(), batch);
            }
    ...
    let mut inventory = DestinationBatchInventory {
        expected_exact: &batch.classifications,
        exact: &mut batch.inventory_exact,
        portable_key: &mut batch.portable_key,
        native_key: &mut batch.native_key,
        normalization: &mut batch.normalization,
        collision_policy: DestinationCollisionPolicy::AllowExternalCollision,
        directory_buffer,
    };
    ...
    if batch.inventory_exact != batch.classifications {
        return pending_publication(io::ErrorKind::InvalidData.into(), batch);
    }
    #[cfg(target_os = "linux")]
    let member_proof = validate_classified_publication_members(
        &batch.stages,
    ...
    for (stage, published) in stages.drain(..).zip(classifications.iter().copied()) {
        if published {
    ...
    if let Err(settlement) =
        TransientEffectToken::settle_classified_batch(&mut classified, operation)
    {
...
impl TransientPublicationBatchObligation {
    ...
    pub fn reconcile(mut self) -> TransientPublicationBatchOutcome {
        let error = std::mem::replace(&mut self.error, io::ErrorKind::Other.into());
        let mut batch = self
            .batch
            .take()
...
mod tests {
    ...
    fn reserve_test_transient(
        ...
        let plan = DestinationBatchPlan::new(vec![LeafName::new(name).expect("transient leaf")])?;
        let mut tokens =
    ...
    fn held_destination_blocks_batch_until_explicit_cancellation() {
        ...
        let root = session.root().expect("root directory");
        let held = root
            .admit_transient_destination(LeafName::new("Held.bin").expect("held destination leaf"))
        ...
        match held.cancel() {
            ...
            TransientDestinationCancelOutcome::Pending(obligation) => {
                panic!(
                    "held destination cancellation remained pending: {}",
                    obligation.error()
                )
            }
        }
        let mut retried = root
            ...
            .into_destinations();
        let retried = retried.pop().expect("retried singleton batch is nonempty");
        match retried.cancel() {
            TransientDestinationCancelOutcome::Cancelled => {}
            TransientDestinationCancelOutcome::Pending(obligation) => {
                panic!(
                    "retried destination cancellation remained pending: {}",
                    obligation.error()
                )
            }
        }
    ...
    fn explicit_destination_cancellation_releases_its_reservation() {
        ...
        match destination.cancel() {
            ...
            TransientDestinationCancelOutcome::Pending(obligation) => {
                panic!(
                    "destination cancellation remained pending: {}",
                    obligation.error()
                )
            }
        }
    ...
    fn discarded_stage_reuses_the_exact_destination_reservation() {
        ...
        let reservation_id = first.token.as_ref().expect("first stage effect token").id;
        let destination = match first.discard() {
            ...
            TransientDiscardOutcome::Pending(obligation) => {
                panic!(
                    "first stage discard remained pending: {}",
                    obligation.error()
                )
            }
        };
        ...
        let second = match destination.create_stage() {
            ...
            TransientStageCreateOutcome::Pending(obligation) => {
                panic!(
                    "second stage creation remained pending: {}",
                    obligation.error()
                )
            }
        };
        assert_eq!(
            second.token.as_ref().expect("second stage effect token").id,
            reservation_id,
        );
        let destination = match second.discard() {
            ...
            TransientDiscardOutcome::Pending(obligation) => {
                panic!(
                    "second stage discard remained pending: {}",
                    obligation.error()
                )
            }
        };
    ...
    fn reserved_token_unwind_is_root_cleanable_no_effect() {
        ...
        let token =
            reserve_test_transient(&authority, &operation, &root, "unwound-reservation.bin")
    ...
    fn move_and_transient_reservations_reject_conflicts_in_either_order() {
...

        {
            ...
            let conflict = reserve_test_transient(&authority, &operation, &root, "destination.BIN");
            match conflict {
...

        {
            ...
            let mut transient = reserve_test_transient(&authority, &operation, &root, "Source.bin")
    ...
    fn unrelated_sibling_tree_reservations_proceed_together() {
        ...
        let mut transient =
            reserve_test_transient(&authority, &operation, &transient_tree, "destination.bin")
    ...
    fn simultaneous_move_and_transient_reservations_admit_exactly_one() {
        ...
        let transient_thread = {
            ...
            thread::spawn(move || {
                ...
                start.wait();
                let reservation = reserve_test_transient(&authority, &operation, &root, "race.BIN");
                finish.wait();
                match reservation {
                    Ok(mut token) => {
                        token
    ...
    #[cfg(target_os = "linux")]
    fn linked_test_stage(root: &Directory, name: &str) -> (TransientStageSealed, u64) {
    ...
    fn assert_retained_transient(
        ...
        let record = state
            .transients
            .get(&id)
            .expect("retained transient record");
        assert!(record.phase == TransientEffectPhase::Abandoned);
    ...
    fn anonymous_stage_publishes_exact_single_link_content() {
        ...
        stage
            .write_all(b"managed payload")
            .expect("stream stage write");
        let sealed = stage.seal().expect("stream stage seal");
        let batch =
            TransientPublicationBatch::new(vec![sealed]).expect("singleton publication batch");
        ...
        assert_eq!(
            std::fs::read(temporary.path().join("published.bin")).expect("published payload read"),
            b"managed payload",
        );
        assert_eq!(
    ...
    fn grouped_publication_releases_every_file_after_one_terminal_outcome() {
        ...
        let files = match batch.publish_create_new() {
            ...
            TransientPublicationBatchOutcome::Pending(obligation) => {
                panic!(
                    "grouped publication remained pending: {}",
                    obligation.error()
                )
            }
        };
    ...
    fn grouped_partial_publication_preserves_original_member_order() {
        ...
        let mut batch =
            TransientPublicationBatch::new(stages).expect("partial grouped publication batch");
        ...
        let destination = match stage.discard() {
            ...
            TransientDiscardOutcome::Pending(obligation) => {
                panic!(
                    "partial stage discard remained pending: {}",
                    obligation.error()
                )
            }
        };
        match destination.cancel() {
            ...
            TransientDestinationCancelOutcome::Pending(obligation) => {
                panic!(
                    "partial destination cancellation remained pending: {}",
                    obligation.error()
                )
            }
        }
    ...
    fn grouped_partial_accepts_stable_unpublished_alias_and_wrong_kind_collisions() {
        for (case, target_name, collision_name, collision_is_directory) in [
            ...
            let destination = match stage.discard() {
                ...
                TransientDiscardOutcome::Pending(obligation) => {
                    panic!(
                        "collision stage discard remained pending: {}",
                        obligation.error()
                    )
                }
            };
            match destination.cancel() {
                ...
                TransientDestinationCancelOutcome::Pending(obligation) => {
                    panic!(
                        "collision cancellation remained pending: {}",
                        obligation.error()
                    )
                }
            }
    ...
    fn grouped_zero_publication_returns_the_intact_no_effect_batch() {
        ...
        std::fs::write(
            temporary.path().join("zero-second.bin"),
            b"second collision",
        )
        ...
        let batch = match classify_publication_batch(
            ...
            TransientPublicationBatchOutcome::Pending(obligation) => {
                panic!(
                    "zero-publication batch remained pending: {}",
                    obligation.error()
                )
            }
        ...
        for stage in batch.into_stages() {
            let destination = match stage.discard() {
                ...
                TransientDiscardOutcome::Pending(obligation) => {
                    panic!(
                        "zero-publication discard remained pending: {}",
                        obligation.error()
                    )
                }
            };
            match destination.cancel() {
                ...
                TransientDestinationCancelOutcome::Pending(obligation) => {
                    panic!(
                        "zero-publication cancellation remained pending: {}",
                        obligation.error()
                    )
                }
            }
    ...
    fn held_transient_rejects_replacement_then_discards_unlinked_payload() {
        ...
        let relink = platform::link_transient_file(
            sealed
                .stage
                .file
                .as_mut()
        ...
        assert_eq!(
            relink
                .expect_err("an unlinked published O_TMPFILE cannot be relinked")
                .kind(),
            io::ErrorKind::NotFound,
        );
        assert!(!temporary.path().join("aba.bin").exists());
        let destination = match sealed.discard() {
            TransientDiscardOutcome::Discarded(destination) => destination,
            TransientDiscardOutcome::Pending(obligation) => {
                panic!(
                    "unlinked transient discard remained pending: {}",
                    obligation.error()
                )
            }
        };
    ...
    fn prepublication_collision_is_deferred_to_publish_and_preserves_the_stage() {
        ...
        let stage = match destination.create_stage() {
            ...
            TransientStageCreateOutcome::Pending(obligation) => {
                panic!(
                    "collision admission remained pending: {}",
                    obligation.error()
                )
            }
        };
        ...
        let preserved = match batch.publish_create_new() {
            ...
            TransientPublicationBatchOutcome::Pending(obligation) => {
                panic!(
                    "collision publication remained pending: {}",
                    obligation.error()
                )
            }
        };
        ...
        match destination.cancel() {
            ...
            TransientDestinationCancelOutcome::Pending(obligation) => {
                panic!(
                    "collision destination cancellation remained pending: {}",
                    obligation.error()
                )
            }
        }
        assert_eq!(
            std::fs::read(temporary.path().join("COLLISION.BIN")).expect("collision payload read"),
            b"user payload",
        );
        drop(root);
        assert!(matches!(session.revoke(), RootRevokeOutcome::Revoked));
