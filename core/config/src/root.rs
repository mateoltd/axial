...
impl AppRootSession {
    ...
    pub fn prepare_runtime_directory(&self) -> io::Result<Directory> {
        self.open_or_create_fixed_directory("runtimes")
    }
    ...
    pub fn prepare_saved_skin_directories(&self) -> io::Result<(Directory, Directory)> {
        let root = self.open_or_create_fixed_directory("skins")?;
        let files = open_or_create_fixed_child(root.clone(), "files")?;
        Ok((root, files))
    }
    ...
    pub async fn begin_reset(self: &Arc<Self>) -> io::Result<AppRootResetAuthority> {
        ...
        let mut outcome = match retry {
            ...
            Some(AppRootResetRetry::Recovery(recovery)) => ResetStartOutcome::Recovery { recovery },
        ...
        loop {
            outcome = match outcome {
                ...
                ResetStartOutcome::Pending(drain) if probes == RESET_SETTLEMENT_MAX_PROBES => {
                ...
                ResetStartOutcome::Recovery { recovery } => {
                    ...
                    match recovery.acknowledge_external() {
                        ResetStartOutcome::Recovery { recovery } => recovery.defer_managed_reset(),
                        outcome => outcome,
                    }
                }
...
fn take_reset_retry(reset_retry: &Mutex<Option<AppRootResetRetry>>) -> Option<AppRootResetRetry> {
    match reset_retry.lock() {
...
fn retain_reset_retry(reset_retry: &Mutex<Option<AppRootResetRetry>>, retry: AppRootResetRetry) {
...
fn open_or_create_fixed_child(
    ...
    match parent.create_directory(&name) {
        ...
        DirectoryCreateOutcome::NoEffect(error) if error.kind() == io::ErrorKind::AlreadyExists => {
...
fn open_exact_fixed_child(parent: &Directory, name: &LeafName) -> io::Result<Option<Directory>> {
...
mod tests {
    ...
    #[test]
    fn saved_skin_preparation_returns_exact_nested_capabilities() {
        let test_root = TestRoot::new("saved-skin-directories");
        let paths = test_root.paths();
        let root = paths.open_root_session().expect("open root session");

        let (skins, files) = root
            .prepare_saved_skin_directories()
            .expect("prepare saved skin directories");

        assert_eq!(
            skins.identity().expect("skins identity"),
            root.root_directory()
                .expect("application root")
                .open_directory(&LeafName::new("skins").expect("skins leaf"))
                .expect("open exact skins directory")
                .identity()
                .expect("opened skins identity")
        );
        assert_eq!(
            files.identity().expect("files identity"),
            skins
                .open_directory(&LeafName::new("files").expect("files leaf"))
                .expect("open exact files directory")
                .identity()
                .expect("opened files identity")
        );
    }
