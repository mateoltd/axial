    ...
    S: FnOnce(),
{
    ...
    if source_relative == target_relative {
        ...
        if manifest_changed {
            ...
            manifest
                .save_with_revalidation(game_dir, || inventory.verify(game_dir, &guarded_paths))?;
        }
...
fn manifest_mod_candidates(manifest: &ContentManifest, source_filename: &str) -> Vec<usize> {
    manifest
        .entries()
        .iter()
        .enumerate()
...
mod tests {
    ...
    fn portable_alias_request_is_rejected_without_claiming_the_exact_entry() {
        ...
        save_managed_mod(&root, "portable-alias", "Stra\u{df}e.jar", true, b"managed");
