    ...
    pub fn validate_provider_pending_projection(
        ...
        let entries = retained
            ...
            .chain(
                pending
                    .iter()
                    .map(|entry| ManifestProjectionEntry::Pending(entry.projection())),
            )
            .collect();
...
fn validate_entry_metadata(
    ...
    validate_required_text("installed timestamp", installed_at, MAX_INSTALLED_AT_BYTES)?;
...
mod tests {
    ...
    fn insert(manifest: &mut ContentManifest, entry: ManifestEntry) -> Option<ManifestEntry> {
    ...
    fn managed_codec_roundtrips_path_free_and_retains_exact_origin() {
        ...
        assert_ne!(
            encoded, compact,
            "the fixture must have distinct formatting"
        );
    ...
    fn enabled_transition_is_atomic_at_the_serialized_bound() {
        ...
        if remaining > 0 && remaining < title_field_bytes {
            index -= 1;
            manifest.entries[index].title =
                Some("x".repeat(MAX_TITLE_BYTES + remaining - title_field_bytes));
    ...
    fn load_rejects_unbounded_or_unsafe_entries() {
        ...
        body["entries"][0]["filename"] = serde_json::Value::String(
            "x".repeat(axial_minecraft::portable_path::MAX_PORTABLE_FILE_NAME_BYTES + 1),
        );
        fs::write(
            &path,
        ...
        let invalid_body = |filename: &str| {
            ...
            invalid["entries"][0]["filename"] = serde_json::Value::String(filename.to_string());
            serde_json::to_vec(&invalid).expect("invalid filename body")
        };
        fs::write(&path, invalid_body("../tracked.jar")).expect("write unsafe entry");
        assert!(ContentManifest::load(&dir).is_err());
    ...
    fn p00_b11_contract_file_owners_require_canonical_sha512_and_positive_exact_size() {
        ...
        for invalid in [serde_json::Value::Null, serde_json::json!("A".repeat(128))] {
