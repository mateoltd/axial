...
pub fn managed_uninstall_observation_paths(
    ...
    for entry in scope.selected.into_iter().chain(scope.dependents) {
        append_unique_paths(&mut paths, &mut seen, managed_entry_variant_paths(entry)?);
    }
    Ok(paths)
}
...
pub fn plan_managed_content_install(
    ...
    if files.is_empty() {
        return Err(ContentError::Invalid(
            "the content install plan is empty".to_string(),
        ));
    }
...
pub fn plan_managed_content_uninstall(
    ...
    let observations = session.observations();
    let index = observation_index(&observations)?;
    let Some(projection) = project_uninstall(observed_manifest.manifest, canonical_ids, &index)?
    else {
        return Ok(None);
    };
    build_projection(
...
fn project_install(
    ...
    for planned in files {
        ...
        if variants.len() != 2 {
            return Err(provider_error(
                "content is not installable as one managed file",
            ));
        }
    ...
    for (index, planned) in files.iter().enumerate() {
        ...
        let payload_id =
            ManagedContentPayloadId::new(&format!("content-{index}")).map_err(core_plan_error)?;
        results.insert(
            target.key(),
    ...
    manifest
        .try_upsert_batch(entries)
        .map_err(|_| provider_error("content metadata conflicts in the managed manifest"))?;
...
fn observed_enabled_state(
    ...
    let enabled_live =
        observed_matches_entry(require_observation(observations, enabled)?.state(), entry);
    let disabled_live =
        observed_matches_entry(require_observation(observations, disabled)?.state(), entry);
    ...
    Ok(enabled_live
        .then_some(true)
        .or(disabled_live.then_some(false)))
}
...
fn observed_matches_entry(state: &ManagedContentObservedState, entry: &ManifestEntry) -> bool {
    match (state, entry.size(), entry.sha512()) {
        (
            ManagedContentObservedState::Exact { size, sha512 },
            Some(expected_size),
            Some(expected),
...
fn require_manifest_snapshot(current: Option<&[u8]>, expected: Option<&[u8]>) -> ContentResult<()> {
    if current != expected {
        return Err(ContentError::Invalid(
...
mod tests {
    ...
    fn selected_dependency_index_preserves_project_and_version_only_semantics() {
        ...
        let dependency =
            |project_id: Option<&str>, version_id: Option<&str>, kind| ContentDependency {
    ...
    fn new_install_projects_one_enabled_download_and_seals() {
        ...
        let paths =
            managed_install_observation_paths(&observed, &planned).expect("install observations");
    ...
    fn replacement_removes_the_owned_stale_variant_and_preserves_disabled_state() {
        ...
        let planning = planning
            .observe_more(paths)
            .expect("observe replacement paths");
        let projection = plan_managed_content_install(&planning, observed, &planned)
    ...
    fn uninstall_refuses_a_live_dependent_but_accepts_the_selected_closure() {
        ...
        let observed = decode_observed_content_manifest(&planning).expect("decoded manifest");
        let paths =
            managed_uninstall_observation_paths(&observed, std::slice::from_ref(&dependency_id))
                .expect("uninstall observations");
        let planning = planning
            .observe_more(paths)
            .expect("observe uninstall scope");
        ...
        let observed = decode_observed_content_manifest(&planning).expect("decoded manifest");
        let projection =
            plan_managed_content_uninstall(&planning, observed, &[dependency_id, dependent_id])
    ...
    fn uninstall_of_absent_owned_variants_seals_as_manifest_only() {
        ...
        let planning = fixture
            ...
            .expect("manifest observation");
        let observed = decode_observed_content_manifest(&planning).expect("decoded manifest");
        let paths = managed_uninstall_observation_paths(&observed, std::slice::from_ref(&entry_id))
            .expect("uninstall observations");
        let planning = planning
            .observe_more(paths)
            .expect("observe absent variants");
        let projection =
            plan_managed_content_uninstall(&planning, observed, std::slice::from_ref(&entry_id))
        ...
        let session = planning
            .finish(Vec::new())
            .expect("manifest-only observation");
