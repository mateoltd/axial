...
impl ExpectedTransferDigests {
    ...
    pub fn from_hex(
        ...
        Ok(Self {
            sha1: sha1
                .map(|value| {
                    parse_hex_digest::<20>(value).ok_or(TransferDigestParseError::InvalidSha1)
                })
                .transpose()?,
            sha512: sha512
                .map(|value| {
                    parse_hex_digest::<64>(value).ok_or(TransferDigestParseError::InvalidSha512)
                })
                .transpose()?,
        })
    }
...
impl CreateOnlyTransferTarget {
    pub fn new(destination: TransientDestination, authority: ManagedTransferAuthority) -> Self {
        Self {
            destination,
            authority,
        }
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TimeoutExceedsRequest(kind) => {
                write!(
                    formatter,
                    "{kind} timeout exceeds the overall request timeout"
                )
            }
...
impl TransferClientConfig {
    ...
    pub fn bounded_pinned_public(
        ...
        let origins = pinned_origins
            ...
            .collect();
        let mut config =
            Self::bounded(connect_timeout, idle_read_timeout, request_timeout, origins)?;
...
impl TransferCleanupObligation {
    ...
    fn reconcile_discard(mut self, outcome: TransientDiscardOutcome) -> TransferCleanupResolution {
        match outcome {
    ...
    fn reconcile_destination_cancel(
        ...
        match outcome {
            TransientDestinationCancelOutcome::Cancelled => TransferCleanupResolution::Discarded {
                report: self.report,
...
fn verified_destination_cancel(
    ...
    match outcome {
        TransientDestinationCancelOutcome::Cancelled => VerifiedTransferDiscardOutcome::Discarded {
            report,
...
fn map_transfer_outcome<T, U>(
    ...
    match outcome {
        ...
        TransferOutcome::CleanupPending(obligation) => TransferOutcome::CleanupPending(obligation),
...
async fn run_transfer(
    ...
    for attempt in 0..MAX_ATTEMPTS {
        ...
        match run_attempt(&client, &url, destination, &contract, cancellation.clone()).await {
...
fn headers_have_identity_encoding(headers: &reqwest::header::HeaderMap) -> bool {
    ...
    std::iter::once(first).chain(values).all(|value| {
        value.to_str().is_ok_and(|encoding| {
            ...
            tokens.peek().is_some() && tokens.all(|token| token.eq_ignore_ascii_case("identity"))
        })
    })
}
...
fn admit_bytes(
    ...
    let additional =
        u64::try_from(additional).map_err(|_| TransferFailureKind::ByteCountOverflow)?;
...
fn final_size_failure(contract: TransferByteContract, observed: u64) -> TransferFailureKind {
    match contract {
...
fn unsettled_after_destination_cancel(
    ...
    match outcome {
        ...
        TransientDestinationCancelOutcome::Pending(obligation) => AttemptOutcome::CleanupPending {
            failure: TransferFailureKind::WorkerStopped,
...
mod tests {
    ...
    fn digest_metadata_is_canonicalized_to_typed_bytes() {
        let parsed =
            ExpectedTransferDigests::from_hex(Some(&"A5".repeat(20)), Some(&"0b".repeat(64)))
    ...
    fn retry_policy_caps_total_attempts_at_eight() {
        ...
        assert!(matches!(
            RetryPolicy::classified(&[MAX_RETRY_DELAY + Duration::from_nanos(1)], retry_network,),
    ...
    fn transfer_client_config_requires_positive_bounded_timeouts() {
        ...
        for (kind, result) in [
            ...
            assert_eq!(result, Err(TransferClientConfigError::ZeroTimeout(kind)));
        }
        for (kind, result) in [
            (
                TransferTimeoutKind::Connect,
        ...
        let loopback_http =
            reqwest::Url::parse("http://127.0.0.1:8080/file").expect("loopback HTTP URL");
        assert_eq!(
            TransferOrigin::from_url(&loopback_http),
            Err(TransferOriginError::UnsupportedScheme)
        );
        assert!(TransferOrigin::from_loopback_http_for_test_support(&loopback_http).is_ok());
        let ipv6_loopback_http =
            reqwest::Url::parse("http://[::1]:8080/file").expect("IPv6 loopback HTTP URL");
        assert!(TransferOrigin::from_loopback_http_for_test_support(&ipv6_loopback_http).is_ok());
        assert_eq!(
    ...
    fn pinned_public_origin_rejects_private_mismatched_and_duplicate_addresses() {
        ...
        for (addresses, error) in [
            ...
            assert_eq!(
                PinnedTransferOrigin::public(origin(), addresses),
                Err(error)
            );
        }
        assert_eq!(
        ...
        assert_eq!(
            PinnedTransferOrigin::public(
                TransferOrigin::from_loopback_http_for_test_support(
                    &reqwest::Url::parse("http://127.0.0.1:8080/file").expect("loopback test URL"),
                )
                .expect("test-only origin"),
                vec![
                    "1.1.1.1:8080"
                        .parse()
                        .expect("public matching-port address")
                ],
            ),
    ...
    fn pinned_public_config_rejects_ambiguous_hosts() {
        ...
        .expect("public matching-port pin");
        let config =
            TransferClientConfig::bounded_pinned_public(connect, idle_read, request, vec![pinned])
    ...
    fn redirect_policy_rejects_cross_origin_and_ninth_redirect() {
        ...
        let cross_origin =
            reqwest::Url::parse("https://release-assets.githubusercontent.com/redirect")
    ...
    async fn writer_channel_closure_interrupts_a_pending_provider_wait() {
        ...
        let wait = wait_for_writer(&mut cancellation, &messages, std::future::pending::<()>());
    ...
    async fn polled_join_drop_cancels_supervisor_and_settles_authority() {
        ...
        std::future::poll_fn(
            |context| match std::future::Future::poll(joined.as_mut(), context) {
    ...
    fn target_cancellation_returns_retained_terminal_authority() {
        ...
        assert!(matches!(
            &outcome,
            TransferTargetCancelOutcome::Cancelled(_)
        ));
        assert_eq!(authority_drops.load(Ordering::SeqCst), 0);
        drop(outcome);
        assert_eq!(authority_drops.load(Ordering::SeqCst), 1);
    ...
    fn content_encoding_accepts_only_absent_or_identity_values() {
        ...
        assert!(!headers_have_identity_encoding(&headers));
        headers.insert(CONTENT_ENCODING, HeaderValue::from_static("identity, gzip"));
        assert!(!headers_have_identity_encoding(&headers));
    ...
    async fn serve_once(body: &'static [u8]) -> (reqwest::Url, tokio::task::JoinHandle<()>) {
        ...
        (
            reqwest::Url::parse(&format!("http://{address}/artifact")).expect("fixture URL"),
            server,
        )
    }
}
