...
fn is_cache_fresh(fetched_at_ms: i64, ttl: Duration) -> bool {
    ...
    fetched_at_ms >= 0 && fetched_at_ms <= now && ((now - fetched_at_ms) as u128) < ttl.as_millis()
}
...
{
    let data = serialize_cache(value)?;
    open_or_create_loader_catalog(operation)?.write_exact_fixture(name.as_str(), &data)
}
...
mod tests {
    ...
    async fn live_fetch_success_survives_cache_write_failure() {
        ...
        let (value, state) = resolve_cached(library.operation(), name, Duration::ZERO, || async {
    ...
    async fn write_cache_replaces_existing_catalog() {
        ...
        let cached =
            read_cache::<Vec<String>>(library.operation(), &name).expect("read replaced cache");
    ...
    async fn write_cache_preserves_directory_destination() {
        ...
        let name = cache_name();
        let catalog =
            open_or_create_loader_catalog(library.operation()).expect("open loader catalog");
        catalog
            .open_or_create_child(name.as_str())
            .expect("create directory at cache destination");
        let next = CachedCatalog {
