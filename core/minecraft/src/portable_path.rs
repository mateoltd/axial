...
const MANAGED_CONTENT_PREFIXES: [&str; 3] =
    [".axial-content-", ".axial-pack-", ".axial-replacement-"];
...
mod tests {
    ...
    fn enforces_utf8_and_utf16_bounds_after_normalization() {
        ...
        assert_eq!(
            PortableRelativePath::new(&format!("{}/{}/c", "a".repeat(255), "b".repeat(255))),
            Err(PortablePathError::Unsafe)
        );
    }
    ...
    fn reserves_only_managed_content_names_and_their_disabled_forms() {
        for value in [
            ...
            assert!(
                managed_content_name_is_reserved(&name),
                "not reserved: {value}"
            );
        }
        for value in [
            ".axial-user-file",
            "config/axial.content.json",
            "my-axial-pack",
        ] {
