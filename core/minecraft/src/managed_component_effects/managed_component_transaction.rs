...
fn recover_component_transaction_blocking(
    ...
    if restart_recovery && ancestor_plan.canonical_records != 0 && !all_ancestors_committed {
        return BlockingDisposition::RecoveryRequired(outcome.map(|(_, guard)| guard));
    }
...
fn retain_runtime_ancestor_entry(
    ...
    let retained = authority
        ...
        .ok_or(ComponentTransactionError)?;
    if retained
        .replace(directory.identity().map_err(tx)?)
        .is_some()
    {
        return Err(ComponentTransactionError);
    }
    ...
    for shard_index in 0..durable_shards {
        ...
        for record in shard.records() {
            ...
            match (canonical, staged) {
                ...
                (None, Some(staged)) if exact_entry_names(&staged, 1).map_err(tx)?.is_empty() => {
                    validate_runtime_ancestor_entry(
                        published.runtime_ancestors.as_ref(),
                        record.ordinal(),
                        &staged,
...
fn cleanup_unjournaled_ancestor_bucket(
    ...
    let bucket = published
        ...
        .open_child(&bucket_name)
        .map_err(tx)?;
    validate_runtime_ancestor_bucket(published.runtime_ancestors.as_ref(), shard_index, &bucket)?;
    ...
    for row_in_shard in 0..slot_prefix {
        ...
        validate_runtime_ancestor_entry(published.runtime_ancestors.as_ref(), ordinal, &slot)?;
        slots.push(slot);
    }
...
fn cleanup_component_settlement(
    ...
    let ancestor_plan = admit_settlement_ancestors(lane, lease, settlement, runtime_ancestors)?;
        ...
        if let Some(bucket) = bucket.as_ref() {
            ...
            for row_in_shard in 0..slot_count {
                ...
                let record = journal
                    ...
                    .get(row_in_shard)
                    .ok_or(ComponentTransactionError)?;
                validate_runtime_ancestor_entry(runtime_ancestors, record.ordinal(), &slot)?;
            }
...
fn validate_settlement_ancestor_terminal(
    ...
    match terminal {
        ComponentTerminalOutcome::Committed => {
            let canonical = canonical.ok_or(ComponentTransactionError)?;
            validate_runtime_ancestor_entry(runtime_ancestors, record.ordinal(), &canonical)?;
        }
