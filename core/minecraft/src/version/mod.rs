...
pub struct VersionBundlePublicationGuardForTest {
    _guard: crate::managed_publication::ManagedRootPublicationTestGuard,
}
...
impl VersionBundleReadGuard {
    pub fn acquire(operation: &ManagedLibraryOperation) -> io::Result<Self> {
        ...
        let root = operation.managed_directory().map_err(io::Error::other)?;
        let lease =
    ...
    fn root_binding(&self) -> io::Result<ManagedDirectoryIdentity> {
        self.lease.root_identity().map_err(publication_read_error)
    }
}

#[cfg(feature = "test-support")]
impl VersionBundlePublicationGuardForTest {
    pub fn acquire(operation: &ManagedLibraryOperation) -> io::Result<Self> {
        operation.revalidate()?;
        let root = operation.managed_directory().map_err(io::Error::other)?;
        let guard = crate::managed_publication::ManagedRootPublicationTestGuard::acquire(root)
...
pub fn scan_versions_snapshot(
    ...
    let versions_revision = versions_root.passive_revision().map_err(io::Error::other)?;
...
#[cfg(test)]
mod tests {
    use super::{VersionScanIssueKind, VersionScanState, VersionStub, resolve_java_version};
