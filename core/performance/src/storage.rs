use axial_fs::{
    Directory, DirectoryCreateOutcome, DirectoryCreateResolution, DirectoryEntry,
    DirectoryIdentity, DirectoryListing, DirectoryMoveOutcome, DirectoryMoveReceipt,
...
impl ManagedInstanceEffectAuthority {
    ...
    fn continuation(&self) -> MutexGuard<'_, Option<ManagedEffectContinuation>> {
        self.inner.continuation.lock().unwrap_or_else(|poisoned| {
...
impl ManagedStorageDirectory {
        ...
        let directory = match self.directory.create_directory(&leaf) {
            ...
            DirectoryCreateOutcome::AppliedUnverified(obligation) => match obligation.reconcile() {
                ...
                DirectoryCreateResolution::Indeterminate(obligation) => {
                    return Err(self
                        .effects
                        .retain(obligation, EffectOwner::retain_directory_create_completion));
                }
            },
        };
    ...
    pub(crate) fn move_child_directory_no_replace(
        ...
        settle_directory_move(
            source
                .into_directory()
                .move_no_replace(destination_parent.directory(), &name),
            &self.effects,
        )
        .map(|directory| Self::from_descendant(directory, self.effects.clone()))
    }
    ...
    pub(crate) fn admit_existing_file_park(
        ...
        self.directory
            .admit_existing_file_park(&original_name, parked.into_park_request(expected_sha256))
    }
...
pub(crate) fn settle_parked_file_removal(
    ...
    match parked.remove() {
        ...
        FileRemovalOutcome::NoEffect { error: _, parked } => Err(owner
            .effects
            .retain(parked, EffectOwner::retain_parked_file_removal)),
        FileRemovalOutcome::AppliedUnverified(obligation) => match obligation.reconcile() {
            FileRemovalResolution::Removed => Ok(()),
            FileRemovalResolution::NoEffect(parked) => Err(owner
                .effects
                .retain(parked, EffectOwner::retain_parked_file_removal)),
            FileRemovalResolution::Indeterminate(obligation) => Err(owner
                .effects
...
pub(crate) fn settle_parked_directory_removal(
    ...
    match parked.remove_empty() {
        ...
        DirectoryRemovalOutcome::NoEffect { error: _, parked } => Err(owner
            .effects
            .retain(parked, EffectOwner::retain_parked_directory_removal)),
        DirectoryRemovalOutcome::AppliedUnverified(obligation) => match obligation.reconcile() {
            DirectoryRemovalResolution::Removed => Ok(()),
            DirectoryRemovalResolution::NoEffect(parked) => Err(owner
                .effects
                .retain(parked, EffectOwner::retain_parked_directory_removal)),
            DirectoryRemovalResolution::Indeterminate(obligation) => Err(owner
                .effects
                .retain(obligation, EffectOwner::retain_directory_removal)),
        },
    }
}
        ...
        current = if create {
            ...
            current.open_child(name)?.ok_or_else(|| {
                io::Error::new(
                    ...
                    "managed storage directory is missing",
                )
            })?
        };
    }
...
fn promote_stage(
    ...
    match sealed.promote_no_replace(destination.directory(), destination.directory(), name) {
...
fn settle_file_park(
    ...
    match outcome {
        ...
        FileParkOutcome::Preserved { error, file: _ } => Err(error),
        FileParkOutcome::AppliedUnverified(obligation) => match obligation.reconcile() {
            ...
            FileParkResolution::Preserved { error, file: _ } => Err(error),
    ...
    match outcome {
        ...
        DirectoryParkOutcome::AppliedUnverified(obligation) => match obligation.reconcile() {
            ...
            DirectoryParkResolution::Indeterminate(obligation) => {
                Err(effects.retain(obligation, EffectOwner::retain_directory_park_removal))
            }
        },
    }
}
