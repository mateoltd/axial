...
- bounded startup out-of-memory classification for standard Java heap/GC failures and line-scoped native JVM fatal-error markers
...
Process output classifies as `out_of_memory` only for bounded JVM failure signatures: `java.lang.OutOfMemoryError`, `GC overhead limit exceeded`, the JVM's insufficient-memory fatal line, or line-scoped `hs_err` native-allocation/out-of-memory markers. Ordinary memory settings, generic memory prose, and mod names containing `memory` do not match. A correlated pre-boot OOM has no automatic recovery directive: Guardian blocks with bounded running-out-of-memory detail and memory-allocation guidance, and the terminal exited session and persisted launch proof use the `StartupFailed` reason. Once a real boot marker has completed startup, an Application-owned non-polling terminal observer requires the exact `out_of_memory` plus `CrashedAfterBoot` record before it authors warned post-boot copy, updates the existing proof, records the instance observation, and releases the transferred session hold; State still owns only process facts and terminal outcome classification.
