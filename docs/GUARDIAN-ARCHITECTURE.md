...
Tier 2 is a separate full-inventory observation primitive for the later R5 idle/on-demand owner. Core validates a borrowed projection over every exact launcher-owned inventory entry with 200,000-entry, 512 MiB-per-file, and 16 GiB aggregate bounds. State owns a global idle epoch and exact foreground barrier for the shared launcher roots. Launch/preflight, installs, performance operations, and installed-version scans register before root or lifecycle work; retained child capabilities keep the barrier active across cancellation-owned scans, Java probes, runtime repair, Tier 1 hashing, install terminal settlement, deferred launch termination, and direct/queued/restarted performance execution. Performance reserves its durable id before start persistence, proves same-State foreground authority at managed admission, and keeps producer plus foreground ownership through terminal journal/status publication; failed terminal publication retries fail closed until exact authority or coordinated shutdown. State mints a move-only Tier 2 ticket only from a current sweep reservation under a brief instance lifecycle acquisition. The reservation owns a producer lease and State cancellation capability, and Execution owns that reservation through physical blocking completion before returning a report and settlement authority.
