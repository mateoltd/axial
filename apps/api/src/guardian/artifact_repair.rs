...
pub(crate) async fn execute_registered_guardian_artifact_repair(
    ...
    match execution.disposition {
        ...
        ArtifactFinishDisposition::Propagate { error, owner } => match owner {
...
async fn finish_artifact_repair(
    ...
    let reconciliation_terminal = match context.admission.terminal(
...
fn retained_disposition_error(
    ...
    match disposition {
        ArtifactFinishDisposition::Complete(proof) => {
            retained_artifact_error(source, None, Some(RetainedArtifactOwner::Completion(proof)))
        }
        ArtifactFinishDisposition::Continue(cause) => {
            retained_artifact_error(source, None, cause.map(RetainedArtifactOwner::Continuation))
        }
        ArtifactFinishDisposition::Propagate { error, owner } => retained_artifact_error(
            source,
            Some(error),
...
mod persistence_contract_tests {
    ...
    async fn accepted_quarantine_checkpoint_is_acknowledged_then_terminalized() {
        ...
        assert_eq!(
            fs::read(parked).expect("acknowledged quarantine"),
            vec![b'x'; EXPECTED_ASSET.len()]
        );
