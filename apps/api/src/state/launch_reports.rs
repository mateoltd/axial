...
impl LaunchReportStore {
    ...
    #[cfg(test)]
    pub(crate) fn load_from_directory_with_coordinator(
        directory: AnchoredRecordDirectory,
        coordinator: PersistenceCoordinator,
    ) -> io::Result<Self> {
        Self::load_from_paths_with_coordinator_and_retention(
    ...
    fn load_from_paths_with_coordinator_and_retention(
        ...
        let (reports, load_issue_count) =
            match proof_retention.retained_session_ids(MAX_STARTUP_REPORTS) {
                Some(protected_session_ids) => {
                    load_report_index_from_directory(&capability, &protected_session_ids)
                }
                None => (BTreeMap::new(), 1),
            };
...
fn load_report_index_from_directory(
    ...
    let mut reports = BTreeMap::<String, (LaunchProofRecord, AnchoredRecordObservation)>::new();
    ...
    for session_id in protected {
        ...
        let name = report_filename(session_id);
        match load_report_candidate_from_directory(directory, std::ffi::OsStr::new(&name)) {
    ...
    let mut names = match directory.names_bounded(MAX_STARTUP_REPORTS.saturating_add(1)) {
        Ok(Some(names)) => names,
        Ok(None) | Err(_) => {
            return admit_report_candidates(reports, bounded_issue_count(issues));
        }
    };
    names.sort();
    ...
    admit_report_candidates(reports, issues)
}

fn admit_report_candidates(
    reports: BTreeMap<String, (LaunchProofRecord, AnchoredRecordObservation)>,
    mut issues: usize,
) -> (BTreeMap<String, LaunchProofRecord>, usize) {
