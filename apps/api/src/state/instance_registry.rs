...
impl InstanceRegistryPersistence {
    ...
    fn claim_with_coordinator(
        ...
        let owner = coordinator
            ...
            .map_err(instance_persistence_error)?;
        let writer = owner.writer(record).map_err(instance_persistence_error)?;
...
impl InstanceDirectoryTopologyProof {
    fn classify(
        ...
        let tombstone_name = LeafName::new(record.tombstone_name.clone()).map_err(|_| {
            invalid_instance_deletion_topology("instance deletion tombstone is not a native leaf")
        })?;
...
fn prove_instance_directory_topology(
    ...
    for instance in &snapshot.instances {
        let pending =
            PendingInstanceDeletion::new(instance.id.clone(), instance.created_at.clone())?;
...
async fn settle_instance_deletion_filesystem(
    ...
    run_state_physical_work(PhysicalIoClass::Heavy, 0, move || match obligation {
        ...
        InstanceDeletionFilesystemObligation::Restore(obligation) => match obligation.reconcile() {
        ...
        InstanceDeletionFilesystemObligation::RemoveParked { record, directory } => match directory
            .remove_tree()
        {
            ...
            DirectoryTreeRemovalOutcome::Indeterminate(obligation) => {
                InstanceDeletionFilesystemResolution::Retained {
                    ...
                    obligation: InstanceDeletionFilesystemObligation::Remove { record, obligation },
                }
            }
        },
        InstanceDeletionFilesystemObligation::RemoveRetained { record, retained } => match retained
            .retry()
        {
            DirectoryTreeRemovalOutcome::Removed => {
                InstanceDeletionFilesystemResolution::Removed(record)
...
impl PreparedInstanceDeletion {
    pub(super) async fn persist(
        ...
        let ticket = match self
            ...
            .persistence
            .writer
            .accept_encoded(self.encoded.clone(), WriteUrgency::Immediate)
        {
            Ok(ticket) => ticket,
            Err(error) => {
            ...
            DirectoryParkResolution::NoEffect(_) => {
                ...
                Err(InstanceDeletionPreparationFailure::Refused(
                    InstanceStoreError::Persistence(io::Error::other(
                        "instance directory park did not take effect",
                    )),
                ))
            }
...
impl InstanceDeletionSettlementRetry {
    pub(super) async fn retry(
        ...
        match settle_instance_deletion_filesystem(obligation).await {
            InstanceDeletionFilesystemResolution::Restored => {
                let settlement =
                    InstanceDeletionFilesystemSettlement::Aborted(AbortedInstanceDeletion {
                        _store: store,
                        _gate: gate,
                    });
                guard.disarm();
...
impl ManagedInstanceContentRoot {
    fn active(incarnation: InstanceLifecycleIncarnation, root: ManagedTreeRoot) -> Self {
        Self {
            incarnation,
            root: Some(root),
            retirement: None,
    ...
    fn begin_retirement(&mut self) -> (Arc<ManagedTreeRetirement>, Arc<AsyncMutex<()>>) {
        if let Some(retirement) = &self.retirement {
            return (Arc::clone(retirement), Arc::clone(&self.settlement));
        }
        let root = self.root.take().unwrap_or_else(|| std::process::abort());
...
impl AppInstanceStore {
    ...
    pub(super) fn managed_game_directory(
        ...
        if let Err(error) = self.require_registered_instance_unchanged(&expected.id, expected) {
            drop((directory, operation));
    ...
    async fn close_instance_content_roots(&self) -> io::Result<OwnedRwLockWriteGuard<()>> {
        let admission = Arc::clone(&self.instance_content_admission)
            .write_owned()
            .await;
        let retirements = {
    ...
    pub(super) async fn prepare_startup_deletion_recovery_with_gate(
            ...
            for record in &snapshot_for_probe.pending_deletions {
                ...
                match (topology.canonical, topology.tombstone) {
                    ...
                    (None, None) => {
                        ...
                        selected = Some((record.instance_id.clone(), Some(record.clone()), None));
                    }
                }
            }
        ...
        let recovery = match (pending, parked) {
            (None, Some(parked)) => {
                InstanceDeletionStartupRecovery::RestoreLive(InstanceDeletionSettlementRetry {
                    guard: InstanceDeletionDropGuard::armed(),
    ...
    pub(super) async fn prepare_delete_with_gate(
        ...
        let files = if let Some(pending) = pending {
            ...
            match attempt {
                InstanceDirectoryParkAttempt::Prepared(PreparedInstanceDeletionFiles::Absent) => {
    ...
    async fn prepare_instance_deletion_files(
        ...
        run_state_physical_work(PhysicalIoClass::Heavy, 0, move || {
            ...
            match directory.park_as(tombstone_name) {
                DirectoryParkOutcome::Parked(directory) => Ok(
                    InstanceDirectoryParkAttempt::Prepared(PreparedInstanceDeletionFiles::Parked {
                        record: pending,
                        directory,
                    }),
                ),
    ...
    async fn reconcile_retry<Gate>(&self, gate: Gate) -> Result<Gate, InstanceStoreError>
    where
        Gate: Send + 'static,
    {
        let retained = self
    ...
    async fn reconcile_obligations<Gate>(&self, gate: Gate) -> Result<Gate, InstanceStoreError>
    where
        Gate: Send + 'static,
    {
...
mod tests {
    ...
    async fn refused_precommit_delete_restores_the_exact_parked_tree() {
        ...
        let tombstone = store.paths().instances_dir().join(&pending.tombstone_name);
        let gate = store
            .acquire_mutation()
            .await
    ...
    async fn delete_files_parks_then_removes_exact_tree_and_clears_marker() {
        ...
        let tombstone_path = store.paths().instances_dir().join(&pending.tombstone_name);
    ...
    async fn startup_completes_pending_tombstone_and_absent_tree_markers() {
        for (name, seed_tombstone) in [
            ...
            let snapshot =
                InstanceRegistrySnapshot::new(Vec::new(), String::new(), vec![pending.clone()])
    ...
    async fn startup_rejects_alias_wrong_kind_unrecognized_and_multiple_tombstones() {
        ...
        let alias_snapshot =
            InstanceRegistrySnapshot::new(vec![alias_instance], String::new(), Vec::new())
        ...
        let gate = wrong_store
            .acquire_mutation()
            .await
            .expect("wrong-kind gate");
        assert!(matches!(
            wrong_store
                .prepare_startup_deletion_recovery_with_gate(gate)
                .await,
        ...
        let first_pending =
            PendingInstanceDeletion::new(first.id.clone(), first.created_at.clone())
                .expect("derive first tombstone");
        let second_pending =
            PendingInstanceDeletion::new(second.id.clone(), second.created_at.clone())
                .expect("derive second tombstone");
        let multiple_snapshot =
            InstanceRegistrySnapshot::new(vec![first, second], String::new(), Vec::new())
    ...
    fn test_store(
        ...
        let root_session = crate::state::test_root_session(&paths);
        let source =
            InstanceStore::from_snapshot(paths, root_session, snapshot).expect("instance source");
    ...
    fn startup_registry_provenance_requires_the_exact_loaded_generation() {
        ...
        changed.push(b'\n');
        std::fs::write(replaced_paths.instances_file(), &changed).expect("replace registry bytes");
        let replaced_directory = AnchoredRecordDirectory::from_directory(
            replaced_root.clone(),
            replaced_root
                .root_directory()
                .expect("replaced root directory"),
        );
