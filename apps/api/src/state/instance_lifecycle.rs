...
impl InstanceLifecycleGuard {
    pub(super) fn into_parts(self) -> (OwnedMutexGuard<()>, InstanceLifecycleIncarnation) {
...
impl InstanceLifecycleGates {
    ...
    async fn acquire_from(
        ...
        loop {
            ...
            return InstanceLifecycleGuard { guard, incarnation };
        }
    }

    pub(super) async fn try_acquire(&self, instance_id: &str) -> Option<InstanceLifecycleGuard> {
        loop {
            ...
            return Some(InstanceLifecycleGuard { guard, incarnation });
        }
    }
...
mod tests {
    ...
    async fn queued_waiter_rejects_the_retired_incarnation_after_locking() {
        ...
        assert!(Arc::ptr_eq(&queued_candidate, &retired_incarnation.gate));
