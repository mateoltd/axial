...
impl MusicCacheOwner {
    ...
    pub(crate) fn prepare_target(&self, name: LeafName) -> io::Result<CreateOnlyTransferTarget> {
    ...
    pub(crate) fn status(&self) -> [bool; MUSIC_TRACK_COUNT] {
        ...
        std::array::from_fn(|index| {
            let name =
                LeafName::new(MUSIC_TRACKS[index].file).expect("fixed music track leaf is valid");
    ...
    fn retain_opened_directory(&self, opened: Option<Directory>) -> io::Result<Option<Directory>> {
...
fn require_exact_track_binding(directory: &Directory, name: &LeafName) -> io::Result<()> {
    exact_track_binding(directory, name)?
        .then_some(())
        .ok_or_else(|| {
            io::Error::new(
...
mod tests {
    ...
    fn same_track_claims_start_once_and_join_the_live_generation() {
        ...
        for _ in 0..8 {
            assert!(matches!(
                owner
                    .claim_flight(MusicTrackId::VaporHalo, &handoff, |_, _, _, _| panic!(
                        "join must not launch another producer"
                    ),)
    ...
    fn separate_tracks_and_owners_start_independently() {
        ...
        for (owner, track) in [
            ...
            assert!(matches!(
                owner
                    .claim_flight(track, &handoff, |_, _, _, _| panic!(
                        "independent running flight must be joined"
                    ),)
    ...
    fn ordinary_failure_admits_retry_but_unsettled_refuses_it() {
        ...
        owner.finish_flight(MusicTrackId::VaporHalo, 1, MusicFlightCompletion::Unsettled);
        assert!(matches!(
            owner
                .claim_flight(MusicTrackId::VaporHalo, &handoff, |_, _, _, _| panic!(
                    "unsettled flight must never restart"
                ),)
    ...
    fn launch_panic_latches_the_track_unsettled() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let _ = owner.claim_flight(MusicTrackId::VaporHalo, &handoff, |_, _, _, _| {
                panic!("launch failed")
            });
        }));
        ...
        assert!(matches!(
            owner
                .claim_flight(MusicTrackId::VaporHalo, &handoff, |_, _, _, _| panic!(
                    "latched flight must never restart"
                ),)
    ...
    async fn request_drop_does_not_end_the_claimed_producer() {
        ...
        let MusicFlightClaim::Join(mut second_waiter) = owner
            .claim_flight(MusicTrackId::VaporHalo, &second_handoff, |_, _, _, _| {
                panic!("later waiter must join the retained producer")
            })
