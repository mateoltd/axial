...
impl ManagedCompositionOwner {
    ...
    pub(super) async fn admit(
        ...
        self.bind_entry_effects(&entry, instance_lifecycle.retained(), lifecycle.clone())
            .await?;
        drop(entry.work_gate.clone().lock_owned().await);
        if entry.phase() == ManagedEntryPhase::Open && entry.effects().require_settled().is_err() {
        ...
        match entry.phase() {
            ...
            ManagedEntryPhase::Latched => recover_admission_owned(admission).await,
        }
    }
    ...
    pub(super) async fn retire(
        ...
        let lifecycle = Arc::new(self.lifecycle.clone().read_owned().await);
        let entry = self.entry(instance_id)?;
        self.retire_entry(entry, instance_lifecycle, lifecycle)
            .await
    }
...
impl AppManagedCompositionAdmission {
    pub(crate) async fn composition_managed_witness_proofs(
        ...
        self.run_owned(
            "composition_managed_witness_proofs",
            |authority, identity, effects, _| async move {
                authority
    ...
    pub(crate) async fn inspect(
        ...
        self.run_owned(
            "inspect",
            move |authority, identity, effects, managed_artifact_epoch| async move {
                authority
    ...
    pub(crate) async fn remove_managed(&self) -> Result<(), ManagedMutationError> {
        self.run_owned(
            "remove",
            |authority, identity, effects, managed_artifact_epoch| async move {
...
mod tests {
    ...
    impl OwnerFixture {
        fn new(name: &str) -> Self {
            Self::new_with_instances(name, [INSTANCE_A.to_string(), INSTANCE_B.to_string()])
        }

        fn new_with_instances(name: &str, instance_ids: impl IntoIterator<Item = String>) -> Self {
    ...
    async fn foreign_instance_lifecycle_authority_is_rejected() {
        ...
        assert!(matches!(
            fixture.owner.admit(INSTANCE_A, admission_lease, true).await,
    ...
    async fn duplicate_retirement_owner_cannot_retain_or_remove_an_entry() {
        ...
        let mut duplicate = Box::pin(fixture.owner.retire(INSTANCE_A, duplicate_lifecycle));
    ...
    async fn sequential_clean_instances_release_effect_owner_capacity() {
        let instance_ids = (1_u64..=300)
            ...
            .collect::<Vec<_>>();
        let fixture =
            OwnerFixture::new_with_instances("clean-owner-eviction", instance_ids.clone());
