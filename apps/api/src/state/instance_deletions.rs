use super::instance_registry::{
    ...
    InstanceDeletionSettlementRetry, InstanceDeletionStartupRecovery, PreparedInstanceDeletion,
};
...
impl InstanceDeletionCoordinator {
    ...
    pub(super) async fn delete_admitted(
        ...
        let auxiliaries = self
            .prepare_auxiliaries(state, lifecycle, instance_id.clone(), delete_files, false)
            .await?;
        let gate = state.instances.acquire_mutation().await?;
        let deletion =
            prepare_deletion(state, auxiliaries, instance_id, delete_files, gate).await?;
    ...
    fn spawn_retained_driver(
        ...
        state: AppState,
        owner: ProducerLease,
        startup_ownership: Option<tokio::sync::watch::Receiver<InstanceDeletionStartupOwnership>>,
    ) {
        let coordinator = self.clone();
    ...
    fn phase(&self) -> InstanceDeletionPhase {
        match self.phase.load(Ordering::Acquire) {
            ...
            value if value == InstanceDeletionPhase::Closed as u8 => InstanceDeletionPhase::Closed,
...
impl InstanceDeletionAuxiliaries {
    async fn commit(&mut self, state: &AppState) -> Result<(), InstanceStoreError> {
        ...
        let known_good =
            std::mem::replace(&mut self.known_good, KnownGoodRetirementProgress::Settled);
        match known_good {
            ...
            KnownGoodRetirementProgress::Retry => {
                if let Err(error) = state.known_good.retry_retirement(&self.instance_id).await {
                    self.known_good = KnownGoodRetirementProgress::Retry;
                    return Err(InstanceStoreError::Persistence(error));
                }
            }
    ...
    loop {
        deletion.phase = match deletion.phase {
            ...
            RetainedInstanceDeletionPhase::PreCommitRestoreRetry {
                ...
                Ok(InstanceDeletionFilesystemSettlement::Aborted(aborted)) => {
                    drop(aborted);
                    return InstanceDeletionAttempt::Failed(original_error);
                }
                Ok(InstanceDeletionFilesystemSettlement::Settled(settled)) => {
                    drop(settled);
                    std::process::abort()
                }
...
fn instance_deletion_closed_error() -> InstanceStoreError {
    InstanceStoreError::Persistence(io::Error::other("instance deletion coordinator is closed"))
}
...
async fn wait_for_startup_waiter_loss(
    ...
    loop {
        let current = *ownership.borrow_and_update();
        match current {
