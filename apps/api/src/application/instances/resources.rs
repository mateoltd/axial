...
impl WorldBackupNamePlan {
    pub(super) fn new(
        ...
        let mut digest = String::with_capacity(WORLD_BACKUP_DIGEST_HEX_BYTES * 2);
        for byte in &Sha256::digest(world_name.as_str().as_bytes())[..WORLD_BACKUP_DIGEST_HEX_BYTES]
        {
        ...
        let temp_names = (1..=WORLD_BACKUP_NAME_ATTEMPTS)
            .map(|attempt| {
                PortableFileName::new_exact(&format!(".axial-world-backup-{nonce}-{attempt}"))
            })
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Self {
            final_names,
...
mod world_backup_name_tests {
    ...
    fn long_world_names_produce_bounded_distinct_plans() {
            ...
            .chain(first_plan.temp_names.iter())
        {
            ...
            assert!(
                candidate.as_str().encode_utf16().count() <= MAX_PORTABLE_FILE_NAME_UTF16_UNITS
            );
        }
...
fn scan_instance_mods(
    ...
    mods.sort_by_key(|entry| {
        managed_content_name_key(&PortableFileName::new(&entry.name).expect("admitted mod name"))
    });
    Ok(mods)
}
