//! Application-owned music cache/download workflow.
...
pub async fn music_status(state: &AppState) -> MusicStatusResponse {
...
pub async fn music_track(
    ...
    if MUSIC_TRACKS.is_empty() {
        return Err(MusicTrackError::NotFound);
    }
    ...
    {
        return Err(MusicTrackError::DownloadFailed {
            body: music_download_failure_body(&error),
        });
    }

    async_fs::read(&local_path)
        .await
        .map(|bytes| MusicTrackBytes {
            bytes,
