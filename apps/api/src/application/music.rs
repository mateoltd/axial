...
pub(crate) async fn music_status(
    state: &AppState,
...
pub(crate) async fn music_track(
    state: &AppState,
...
fn settle_no_effect_publication(
    ...
    match verified.discard() {
        VerifiedTransferDiscardOutcome::Discarded { .. } => recheck_exact_occupant(owner, name),
...
mod tests {
    use super::*;
    #[cfg(target_os = "linux")]
    use crate::state::{AppStateInit, InstallStore, MusicTestSources, SessionStore};
    #[cfg(target_os = "linux")]
    ...
    async fn music_status_requires_a_live_request_handoff() {
        let source =
            reqwest::Url::parse("http://127.0.0.1:9/music").expect("fixed loopback test URL");
    ...
    async fn serve_once(body: &'static [u8]) -> (reqwest::Url, tokio::task::JoinHandle<()>) {
        ...
        (
            reqwest::Url::parse(&format!("http://{address}/music")).expect("music fixture URL"),
            server,
        )
    }
}
