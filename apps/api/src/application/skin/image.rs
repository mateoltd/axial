...
pub(super) fn decode_skin_png(bytes: &[u8]) -> Result<DecodedSkinPng, ApiError> {
    decode_skin_png_validated(bytes).map_err(|error| match error {
        SkinPngValidationError::TooLarge => {
            json_error(StatusCode::PAYLOAD_TOO_LARGE, "skin upload is too large")
        }
        ...
        SkinPngValidationError::InvalidDimensions => {
            json_error(StatusCode::BAD_REQUEST, "skin image must be 64x64 or 64x32")
        }
    })
}

fn decode_skin_png_validated(bytes: &[u8]) -> Result<DecodedSkinPng, SkinPngValidationError> {
...
fn decode_skin_png_with_budget(
    ...
    let frame = reader
        ...
        .map_err(|_| SkinPngValidationError::InvalidPng)?;
    if frame.width != SKIN_WIDTH || !matches!(frame.height, LEGACY_SKIN_HEIGHT | SKIN_HEIGHT) {
        return Err(SkinPngValidationError::InvalidDimensions);
    }
