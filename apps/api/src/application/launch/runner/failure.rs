...
const ROSETTA_REQUIRED_PUBLIC_MESSAGE: &str = "This Minecraft version needs Rosetta 2 on an Apple Silicon Mac. Install Rosetta 2 by running `softwareupdate --install-rosetta --agree-to-license` in Terminal, then launch again.";
...
pub(super) async fn fail_launch(
    ...
    let public_message =
        sanitize_live_launch_failure_message_for_class(failure.class, failure.message);
...
pub(in crate::application::launch) fn sanitize_live_launch_failure_message(
    ...
    sanitize_live_launch_failure_message_for_class(LaunchFailureClass::Unknown, message)
}

fn sanitize_live_launch_failure_message_for_class(
    class: LaunchFailureClass,
    message: &str,
) -> String {
    if class == LaunchFailureClass::RosettaRequired {
        return ROSETTA_REQUIRED_PUBLIC_MESSAGE.to_string();
    }
...
mod tests {
    ...
    #[test]
    fn rosetta_display_text_without_typed_class_is_not_trusted() {
        let error = axial_minecraft::JavaRuntimeLookupError::RosettaRequired {
            component: "jre-legacy".to_string(),
        };
        let public_message =
            sanitize_live_launch_failure_message(&format!("resolve java: {error}"));
