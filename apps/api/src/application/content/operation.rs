    ...
    G: FnMut(ExecutionDownloadFact) + Send + 'static,
{
    ContentOperationTask::spawn_cancellable(producer, move |cancellation| async move {
        execute_content_install(state, request, cancellation, on_progress, on_download_fact).await
    })
}
...
{
    ContentOperationTask::spawn_cancellable(producer, move |cancellation| async move {
        execute_content_uninstall(state, instance_id, canonical_ids, cancellation, on_progress)
            .await
    })
}
    ...
    F: FnMut(DownloadProgress),
{
    ...
    let planning = observe_more_if_needed(planning, missing).await?;
    let Some(projection) =
        plan_managed_content_uninstall(&planning, observed_manifest, &canonical_ids)
            .map_err(super::content_execution_error)?
    else {
        return Ok(());
    };
...
async fn pinned_transfer_client(
    ...
    let pinned =
        PinnedTransferOrigin::public(origin, addresses).map_err(|_| content_download_failed())?;
...
fn content_authority_error(error: std::io::Error) -> ContentExecutionError {
    match error.kind() {
        std::io::ErrorKind::NotFound => {
            ContentExecutionError::from(json_error(StatusCode::NOT_FOUND, "instance not found"))
        }
...
mod tests {
    ...
    fn dns_addresses_are_capped_at_the_pinning_bound() {
        let addresses = (1_u8..=40).map(|suffix| SocketAddr::from(([203, 0, 113, suffix], 443)));
