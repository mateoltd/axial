...
use axial_content::{
    ...
    LiveManagedContent, ResolutionConflict, ResolutionConflictKind, ResolutionConflictReason,
    ResolutionError, ResolutionLimitKind, ResolutionReason, ResolutionSelection, ResolutionTarget,
    resolve_content,
};
...
pub async fn resolve(
    ...
    resolve_core(state, target, selections, manifest, live_content)
        .await
...
async fn resolve_core(
    ...
    let selections: Vec<ResolutionSelection> = selections
        ...
        .collect();
    resolve_content(state.content(), target, &selections, manifest, live_content).await
}
