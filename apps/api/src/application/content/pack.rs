...
#[cfg(test)]
use axial_content::ManagedContentFileName;
use axial_content::{
...
impl PreparedPackManifest {
    fn materialize(
        ...
        if installed_by_hash.len() < self.entries.len() {
            return Err(axial_content::ContentError::ProviderMetadataInvalid(
                "authenticated modpack content cardinality changed before finalization".to_string(),
            ));
        }
...
async fn prepare_pack_manifest(
    ...
    if !by_hash.is_empty() {
        ...
        let resolved = state
            ...
            .await
            .map_err(content_execution_error)?;
        reject_duplicate_pack_projects(&resolved, &by_hash, record_pack_root.then_some(pack_id))?;
...
mod tests {
    ...
    fn prepared_pack_entry_for(project: &str, filename: &str, hash: &str) -> PendingManifestEntry {
        PendingManifestEntry::managed_file(
    ...
    fn stale_pack_cleanup_preserves_a_manual_replacement() {
        ...
        assert!(
            verified_stale_pack_files(&root, &[entry], &ProtectedManagedPaths::default(),).is_err()
        );
        assert_eq!(
