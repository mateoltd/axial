...
async fn terminal_journal_failure_retries_before_status_and_stream_release() {
    ...
    assert_eq!(
        state
            ...
            .id,
        strict_operation_id(&install_id)
    );
    let journal = state
        .journals()
        .get(&strict_operation_id(&install_id))
...
async fn pre_effect_journal_acceptance_failure_exits_without_retry_or_filesystem_effect() {
    ...
    assert!(state.journals().get(&status.id).is_none());
    assert_eq!(
        state
            .performance_operations()
            .get(&status.id)
...
async fn restart_terminalizes_mismatched_gated_journal_without_spinning() {
    ...
    assert!(
        performance_operation_status_by_id(&reloaded, &status.id)
            .await
...
async fn repeated_mismatches_terminalize_at_full_journal_capacity() {
    ...
    for status in &statuses {
        ...
        assert!(
            performance_operation_status_by_id(&state, &status.id)
                .await
...
async fn fresh_mismatch_reconciliation_remints_matching_identity_collision() {
    ...
    let colliding_id =
        crate::state::contracts::OperationId::try_from("op-ffffffff-ffff-4fff-8fff-ffffffffffff")
            .expect("valid collision id");
    let fresh_id =
        crate::state::contracts::OperationId::try_from("op-00000000-0000-4000-8000-000000000000")
...
async fn provider_resolution_failure_terminalizes_before_effect_started() {
    ...
    let public = performance_operation_status_by_id(&fixture.state, &status.id)
        .await
...
async fn fresh_download_failure_has_no_effect_marker_or_rollback_proof() {
    ...
    let proof = performance_operation_status_by_id(&fixture.state, &status.id)
        .await
...
async fn startup_rejects_mismatched_terminal_and_intent_without_proof() {
    for (name, checkpoint) in [
        ...
        assert!(
            reconciliation_journal_for_str(&state, &install_id).is_some_and(|journal| {
                journal.status == crate::state::contracts::OperationStatus::Failed
            })
        );
        let _ = fs::remove_dir_all(root);
...
async fn startup_corrects_terminal_status_with_active_same_id_journal() {
    ...
    assert!(
        performance_operation_status_by_id(&second_restart, &status.id)
            .await
...
async fn invalid_timestamps_are_excluded_from_operation_endpoints() {
    ...
    let operation_key = strict_operation_id(&operation_id);
...
fn test_mismatched_performance_journal(
    ...
    use crate::state::contracts::{
        CommandKind, JournalId, OperationJournalEntry, OperationJournalStep, OperationPhase,
        OwnershipClass, StabilizationSystem, TargetKind,
    };
...
fn write_persisted_performance_status(
    ...
    fs::write(
        crate::state::performance_operations::operation_path(&dir, &status.id),
...
fn operation_journal_snapshot_path(root: &FsPath) -> PathBuf {
    test_paths(root).operation_journal_file().to_path_buf()
}

fn rewrite_performance_journal_targets(
    root: &FsPath,
    operation_id: &impl ToString,
    target_id: &str,
) {
...
async fn performance_operation_status_by_id(
    state: &AppState,
    operation_id: &crate::state::contracts::OperationId,
) -> Result<PerformanceOperationStatusResponse, (StatusCode, Json<serde_json::Value>)> {
    performance_operation_status(state, &operation_id.to_string()).await
}
