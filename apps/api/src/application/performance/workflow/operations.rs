...
use crate::state::performance_operations::{
    ...
    PERFORMANCE_EFFECT_STARTED_STATE, PerformanceOperationIdReservation,
    PerformanceOperationJournalIdentity, PerformanceOperationPayload,
    PerformanceOperationStartError, PerformanceOperationStatus, PerformanceOperationStoreError,
    normalized_operation_timestamp, sanitize_operation_error,
};
use crate::state::{
...
pub(super) async fn queue_performance_operation(
    ...
    let operation_id = reserve_available_performance_operation_id(&state).map_err(|_| {
        performance_operation_start_error(PerformanceOperationStartError::IdentityExhausted)
    })?;
...
fn reserve_available_performance_operation_id(
    ...
    for _ in 0..PERFORMANCE_RECONCILIATION_ID_ATTEMPTS {
        ...
        if state.journals().get(reservation.operation_id()).is_none() {
    ...
    Mint: FnMut() -> OperationId,
{
    for _ in 0..PERFORMANCE_RECONCILIATION_ID_ATTEMPTS {
        ...
        let expected = mismatched_reconciliation_entry(&operation_id, parent_operation_id, action);
    ...
    status.state == "failed"
        ...
        && status
            ...
            .and_then(|identity| {
                Some((
                    operation_action_from_name(&identity.action)?,
                    identity.reconciliation_operation_id.clone()?,
                ))
            })
            .is_some_and(|(action, reconciliation_id)| {
                mismatched_reconciliation_committed(state, &reconciliation_id, &status.id, action)
            })
}
...
async fn record_performance_progress_status(
    state: &AppState,
    operation_id: &OperationId,
    phase: &str,
) {
...
pub(super) async fn begin_performance_operation_journal(
    ...
    if linked_operation_id.is_some() {
        let Some(status) = state.performance_operations().get(&operation_id).await else {
    ...
    if allow_existing && let Some(existing) = state.journals().get(&operation_id) {
