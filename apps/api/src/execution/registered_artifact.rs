...
use axial_fs::{
    ...
    FileRevision, LeafName, ParkedFile, SealedStagedFile, StageDiscardObligation,
...
impl RegisteredArtifactMutationCapability {
    ...
    pub(crate) async fn quarantine_existing(
        ...
        let quarantine_name =
            LeafName::new(format!(".axial-quarantine-{operation_id}")).map_err(|_| {
                mutation_error(
                    io::Error::new(
        ...
        match result {
            ...
            Err(failure) => Err(RegisteredArtifactMutationError {
                facts: vec![file_fact(failure.kind, Some(operation_id.clone()), target)],
...
impl RegisteredArtifactMutationProof {
    pub(crate) async fn validate(self) -> Result<Self, RegisteredArtifactEffectPreservationError> {
        ...
        .unwrap_or_else(|error| {
            Err(
                RegisteredArtifactEffectPreservationError::PublishedUnresolved {
...
impl RegisteredArtifactQuarantinePreservation {
    pub(crate) async fn acknowledge_preserved(
        ...
        .unwrap_or_else(|error| {
            ...
            Err(
                RegisteredArtifactEffectPreservationError::AcknowledgementUnresolved {
                    error: io::Error::other(error),
                    _root_session: retained_root_session,
                },
            )
...
impl std::fmt::Display for RegisteredArtifactEffectPreservationError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Create { .. } => {
                formatter.write_str("registered artifact stage creation remains unsettled")
            }
            Self::Park { .. } => {
                formatter.write_str("registered artifact quarantine remains unsettled")
            }
            Self::ParkAcknowledgement { .. } => {
                formatter.write_str("registered artifact quarantine acknowledgement failed")
            }
            Self::AcknowledgementUnresolved { .. } => formatter
                .write_str("registered artifact quarantine acknowledgement remains unresolved"),
            Self::Published { .. } => {
                formatter.write_str("published registered artifact requires reconciliation")
            }
            Self::PublishedUnresolved { .. } => {
                formatter.write_str("published registered artifact validation remains unresolved")
            }
            Self::Promotion { .. } => {
                formatter.write_str("registered artifact promotion remains unsettled")
            }
            Self::Discard { .. } => {
                formatter.write_str("registered artifact stage discard remains unsettled")
            }
        }
    }
}
...
fn classify_registered_artifact(
    ...
    match location.parent.open_file(&location.leaf) {
        Ok(file) => match verify_open_registered_artifact(file, expected_sha1, expected_size) {
            RegisteredArtifactVerification::Exact(file, revision) => Some(
                RegisteredArtifactPhysicalState::Exact(RegisteredArtifactObservedExactProof {
                    file,
                    revision,
...
fn verify_open_registered_artifact(
    ...
    let revision = match file.revision() {
        ...
        Err(error) => {
            return RegisteredArtifactVerification::Uncertain { file, error };
        }
    };
    if revision.size() != expected_size {
        return match file.validate_revision(&revision) {
            ...
            Err(error) => RegisteredArtifactVerification::Uncertain { file, error },
        };
    }
    let digest = {
...
fn settle_artifact_park(
    ...
    match outcome {
        ...
        FileParkOutcome::Preserved { error, file: _ } => Err(refused_worker_failure(
            error,
            ExecutionFactKind::PrimitiveRefused,
        )),
        FileParkOutcome::AppliedUnverified(obligation) => {
            ...
            match obligation.reconcile() {
                ...
                FileParkResolution::Preserved { error, file: _ } => Err(refused_worker_failure(
                    error,
                    ExecutionFactKind::PrimitiveRefused,
                )),
...
fn stage_and_promote_registered_artifact(
    ...
    let promotion = sealed.promote_no_replace(&location.parent, &location.parent, &location.leaf);
...
fn settle_artifact_promotion(
    ...
    match outcome {
        ...
        FilePromotionOutcome::NoEffect { error, staged } => Err(discard_sealed_after_failure(
            staged,
            error,
        ...
        FilePromotionOutcome::AppliedUnverified(obligation) => {
            ...
            match (*obligation).reconcile() {
                ...
                FilePromotionResolution::NoEffect(staged) => Err(discard_sealed_after_failure(
                    staged,
                    source,
                    ExecutionFactKind::DownloadPromotionFailed,
                    &root_session,
                )),
                FilePromotionResolution::Indeterminate(obligation) => Err(ArtifactWorkerFailure {
                    kind: ExecutionFactKind::DownloadPromotionFailed,
                    source,
                    preservation: Some(RegisteredArtifactEffectPreservationError::Promotion {
                        obligation,
                        _root_session: root_session,
                    }),
                }),
...
fn fact_error(
    ...
    fact_error_with_source(
        ...
        io::Error::other(match kind {
            ...
            ExecutionFactKind::PrimitiveRefused => "registered artifact primitive was refused",
...
mod tests {
    use super::{
        ArtifactStreamFailure, ArtifactWorkerFailure, ExecutionFactKind,
        ...
        RegisteredArtifactQuarantineOutcome, selected_download_mutation_error,
    };
    ...
    async fn quarantine_receipt_is_acknowledged_only_after_the_caller_accepts_it() {
        ...
        assert_eq!(
            fs::read(&parked).expect("read preserved artifact"),
            b"corrupt"
        );
    ...
    async fn target_appearing_during_download_is_not_replaced_and_stage_is_discarded() {
        ...
        assert_eq!(
            fs::read(&destination).expect("read concurrent target"),
            b"concurrent"
        );
        assert!(
    ...
    async fn already_exact_quarantine_outcome_leaves_the_target_in_place() {
        ...
        assert_eq!(
            fs::read(base.join(&relative)).expect("read exact target"),
            b"exact"
        );
        assert!(
            !base
        ...
        let error: RegisteredArtifactObservedExactValidationError = match proof.validate().await {
            Err(error) => error,
            Ok(_) => {
    ...
    async fn existing_target_is_refused_before_any_http_request() {
        ...
        assert_eq!(
            listener
                .accept()
                .expect_err("provider must receive no request")
                .kind(),
    ...
    fn exact_proof_fixture(label: &str) -> (PathBuf, PathBuf, Arc<axial_config::AppRootSession>) {
        let base = std::env::temp_dir().join(format!(
            "axial-registered-artifact-{label}-{}",
            uuid::Uuid::new_v4()
        ));
