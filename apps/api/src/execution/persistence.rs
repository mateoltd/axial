            ...
            OwnerScope::Directory => {
                self.directory.as_ref().and_then(Weak::upgrade).is_some()
                    || self
                        .records
                        .iter()
                        .any(|(_, owner)| owner.upgrade().is_some())
            }
...
impl PersistenceOwnerLease {
    ...
    pub(crate) async fn close(&self) -> Result<(), PersistenceError> {
        ...
        if succeeded && let Err(error) = settle_owner_capabilities(self.inner.clone(), lanes).await
        {
...
impl AtomicSnapshotWriter {
    pub(crate) async fn delete(&self) -> Result<(), PersistenceError> {
        ...
        let transition = RecordDeleteTransition::new(self.lane.clone(), previous, previous_failure);
        self.flush().await?;
...
fn run_blocking_write(lane: &PathLane, payload: WritePayload) -> BlockingWriteOutcome {
    ...
    for lane in lanes {
        loop {
            ...
            let deletion_pending = {
                let state = lane.state.lock().expect("persistence lane lock poisoned");
                match state.lifecycle {
                    RecordLifecycle::Deleting => true,
                    RecordLifecycle::DeletePending => {
                    ...
                    RecordLifecycle::Open | RecordLifecycle::Deleted => false,
                }
            };
            if !deletion_pending {
                break;
...
mod tests {
    ...
    fn fixture(
        ...
        let owner = coordinator
            ...
            .expect("claim owner");
        let writer = test_writer(&owner, &root.join("snapshot.json")).expect("create writer");
    ...
    async fn accepted_work_survives_dropping_owner_and_writer_handles() {
        ...
        reclaimed
            .writer(target)
            .expect("reclaim writer after worker completion");
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    async fn duplicate_owner_is_rejected_until_every_owner_bound_lane_is_gone() {
        ...
        let owner = coordinator
            .claim_directory(directory.clone())
            .expect("claim owner");
        let writer = test_writer(&owner, &root.join("snapshot.json")).expect("writer");
    ...
    async fn successful_close_releases_owner_and_paths_for_immediate_reclaim() {
        ...
        for revision in 0..128 {
            let writer = test_writer(&owner, &destination).expect("claim snapshot path");
            writer
    ...
    async fn writer_rejects_capability_scope_and_owner_contract_collisions() {
        ...
        let _destination_writer = test_writer(&owner, &destination).expect("claim destination");
    ...
    async fn owner_flushes_and_closes_all_live_child_lanes() {
        ...
        let owner = coordinator
            .claim_directory(test_directory(&root))
            .expect("claim owner");
        let first = test_writer(&owner, &root.join("first.json")).expect("first writer");
        let second = test_writer(&owner, &root.join("second.json")).expect("second writer");
        drop(
            first
