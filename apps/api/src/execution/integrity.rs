...
mod tests {
    ...
    async fn close_fixture(state: AppState, root: PathBuf) {
        state
            .shutdown()
            .await
            .expect("shutdown integrity fixture state");
        drop(state);
        let _ = fs::remove_dir_all(root);
    }
    ...
    async fn registered_artifact_missing_and_corrupt_repairs_settle_exact_registered_terminal() {
        for (label, corrupt) in [("missing", false), ("corrupt", true)] {
            ...
            let authorization = findings
                ...
                .expect("repair authorization");
            let operation_id =
                OperationId::deterministic_test(format!("registered-artifact-{label}"));
            let admission = state
                .admit_registered_artifact_repair(
                    authorization,
                    operation_id.clone(),
            ...
            assert_eq!(
                state
                    .admit_registered_artifact_repair(
                        authorization,
                        OperationId::deterministic_test(format!(
                            "registered-artifact-{label}-suppressed"
                        )),
                        chrono::Duration::minutes(15),
                    )
                    .await
