use axial_api::application::skin::{SKIN_PNG_MAX_BYTES, SkinPngValidationError, validate_skin_png};
...
use tauri::{DragDropEvent, Emitter, PhysicalPosition, Window};
...
impl NativeSkinDropCoordinator {
    ...
    pub(crate) fn ensure_ingress_open(&self) -> Result<(), String> {
        let _lifecycle = self.lifecycle_gate.lock().expect(SKIN_DROP_LOCK_INVARIANT);
        if self
            .shared
            .lock()
...
fn expire_pending(shared: &Arc<Mutex<NativeSkinDropState>>, generation: u64, token: &str) {
    let (pending, expiry_task) = {
...
fn emit_drag(
    ...
    let error = match error {
        ...
        Some("Skin image must be 64x64 or 64x32.") => Some("Skin image must be 64x64 or 64x32."),
...
mod tests {
    ...
    async fn newer_failed_drop_revokes_the_previous_skin_drop_token() {
        ...
        assert!(NativeSkinFileAdmission::admit(&root_session, dir.join("missing.png")).is_err());
    ...
    fn terminal_fence_during_drag_admission_returns_bounded_closed_error() {
        ...
        let admission = NativeSkinFileAdmission::admit(&root_session, path).expect("admit skin");
        {
            let _lifecycle = lifecycle_gate.lock().expect(SKIN_DROP_LOCK_INVARIANT);
            coordinator.fence_for_terminal_while_lifecycle_locked();
        }
